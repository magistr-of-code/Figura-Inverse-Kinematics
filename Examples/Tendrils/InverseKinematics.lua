...
---@param newRootPoint Vector3
---@return self
function armatureSystem:moveTo(newRootPoint)

    for _, value in pairs(self.armatures) do
        local localStartPos = value.startVec:copy():sub(self.armatures[1])
        local localEndPos = value.endVec:copy():sub(self.armatures[1])

        value.startVec = localStartPos:add(newRootPoint)

        value.endVec = localEndPos:add(newRootPoint)
    end
